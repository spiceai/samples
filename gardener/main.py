...
def maintain_garden_moisture_content(garden):
    with open('garden_data.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["time", "temperature", "moisture"])
    
        while True:
            # Observe garden
            print(f"Time: {garden.get_time_unix_seconds()} Temperature: {round(garden.get_temperature(),3)} Moisture: {round(garden.get_moisture(),3)}")

            # Post observation
            writer.writerow([garden.get_time_unix_seconds(), round(garden.get_temperature(),3), round(garden.get_moisture(),3)])
